...
public class TestCaseContext {
   ...
   * 
   * @param masterWindowHandle
   *          String
   */
  public final void setMasterWindowHandle(final String masterWindowHandle) {
  ...
  public final int getWindowNumberByHandle(final String handle) {
    return this.windowHandleToNumberMap.get(handle) != null ? this.windowHandleToNumberMap
        .get(handle) : 0;
  }
  ...
  public final void processWindowData() {
    ...
    for (String handle : driver.getWindowHandles()) {
      ...
      boolean isNew = (((windowNumber != null) && (windowNumber != 0)) ? false
          : true);
      if (isNew) {
    ...
    if (this.getWindowCount() > 0) {
      ...
      LOG.info("Current window number = "
          + this.getWindowNumberByHandle(this.getDriver().getWindowHandle()));
    }
  }
