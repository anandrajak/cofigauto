...
public class AutomationTestSuite {
	...
	public AutomationTestSuite(String excelFile) throws IOException {
		...
		if(!file.exists()){
			throw new ToolRuntimeException("Excel file doesnt exists at location "+excelFile);
		}
		
		HSSFWorkbook testCaseExcel = new HSSFWorkbook(new FileInputStream(excelFile));
			...
			for (; count <= 1000; count++) {
				...
				if (LOG.isDebugEnabled()) {
					...
					if (row != null) {
						...
						LOG.debug(" StringUtils.isBlank(row.getCell(0).toString())........ "
				...
				if (step.isRun()) {
					...
					LOG.info("Test case run is N. Skipping test at row " + (count - 1));
				}
			}
			LOG.info("Total test cases to be executed are " + tests.size() + ". Out of configured " + (count - 1)
					+ " test cases");
		} else {
