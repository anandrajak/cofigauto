...
public class WaitUntil implements ActionCommand {
	...
	final class TestCasePredicate implements Predicate<WebDriver> {
		 ...
		 * @param testCaseStep AutomationTestCaseStep
		 */
		TestCasePredicate(AutomationTestCaseStep testCaseStep) {
		...
		public boolean apply(WebDriver driver) {
			...
			try {
				element = IdentifierType.getElementForIdentifier(testCaseStep.getIdentifierType(),
						testCaseStep.getIdentifier(), testCaseStep.getData(), driver);
			} catch (NoSuchElementException exp) {
				LOG.warn("Element not yet found. Identifier= '" + testCaseStep.getIdentifier() + "'");
				foundElement = false;
			} catch (Exception e) {
				LOG.warn("Exception occurred in wait until condition. Exiting condition.", e);
				foundElement = false;
			}
			if (element != null) {
				foundElement = true;
			}
			LOG.info("Checking condition in wait until ....... returning " + foundElement);
			return foundElement;
		}
	}
	...
	public TestStatus execute(AutomationTestCaseStep step, TestCaseContext context) {
		...
		if (StringUtils.isNotBlank(step.getIdentifierType()) && StringUtils.isNotBlank(step.getIdentifier())) {
			try {
				new WebDriverWait(context.getDriver(), StringUtils.isNotBlank(step.getData()) ? Double.valueOf(
						step.getData()).longValue() : 5, 1000).until(new TestCasePredicate(step));
				status.setStatus(ValidTestStatus.PASS);
			} catch (TimeoutException texp) {
				LOG.error("wait until Timed out. NOT FOUND Identifier= '" + step.getIdentifier() + "'");
