...
public class VerifyProperty implements ActionCommand {
	...
	private static final Logger LOG = LoggerFactory.getLogger(VerifyProperty.class);
			...
			if (StringUtils.isNotBlank(name) && StringUtils.isNotBlank(value)) {
				...
				if (element != null) {
					...
					if (StringUtils.isNotBlank(attributeValue) && attributeValue.equals(value)) {
						...
						status.setStatusDesc("Got attribute value '" + attributeValue + "'. Expected was '" + value
								+ "'. attribute name '" + name + "'");
					}
				} else {
					status.setStatus(ValidTestStatus.FAIL);
